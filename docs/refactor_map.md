...
| 13 | 1994 | `crates/rustwx-io/src/lib.rs` | Fetch requests/results, probing, extraction, selector handling, and cache-facing IO helpers. | Continue splitting fetch/probe/grib modules after product characterization tests. |
| 14 | 1964 | `crates/rustwx-products/src/dataset_export.rs` | Native dataset export and training-bundle shaping. | Split plan/materialize/report helpers after manifest snapshots. |
| 15 | 1907 | `crates/rustwx-io/src/earth2_archive.rs` | AIFS/Earth2 archive member/stat selection, archive paths, and validation. | Keep with IO split but preserve archive path behavior exactly. |
| 16 | 1874 | `crates/rustwx-core/src/lib.rs` | Shared core types for grids, products, selectors, models, fields, and bundles. | Continue splitting only if public compatibility is protected by re-exports. |
| 17 | 1873 | `crates/rustwx-cli/src/bin/forecast_now.rs` | One-shot operational orchestrator across products and lanes. | Candidate for main CLI subcommand once shared args are extracted. |
...
`crates/rustwx-products/src/tests.rs` owns crate-root product public-surface
...
tests with the same parent-private test-module pattern.
`crates/rustwx-products/src/satellite/batch/tests.rs` owns satellite product
slug parsing, required-channel dedupe/sorting, S3 list XML parsing, GOES bucket
and sector aliases, full-disk defaults, and high-resolution visible-channel
opt-in regression tests with the same parent-private test-module pattern.
...
`rustwx-cross-section` owns cross-section paths, section sampling, vertical axes,
terrain masking, palette/style metadata, and lightweight cross-section rendering.
Keep product-specific field construction in `rustwx-products::cross_section` and
preserve the renderer facade while splitting drawing helpers.
`crates/rustwx-cross-section/src/render/tests.rs` owns cross-section render
regression tests for header/legend/terrain output, isotherm overlays, request
builder overrides, section-relative wind vectors, wind-barb point selection,
canvas text rendering, and badge sizing with parent-private access through the
...
`crates/rustwx-cli/src/bin/forecast_now/tests.rs` owns pinned-run request,
