...
use rustwx_cuda_core::{ContextHandle, Result, global};
...
fn regions() -> [Region; 8] {
    [
        Region { name: "conus",                 off_x: 0,    off_y: 0,   nx: CONUS_NX, ny: CONUS_NY },
        Region { name: "pacific_northwest",     off_x: 0,    off_y: 700, nx: 442, ny: 316 },
        Region { name: "california_southwest",  off_x: 0,    off_y: 200, nx: 501, ny: 390 },
        Region { name: "rockies_high_plains",   off_x: 400,  off_y: 350, nx: 472, ny: 464 },
        Region { name: "southern_plains",       off_x: 550,  off_y:  80, nx: 560, ny: 576 },
        Region { name: "great_lakes",           off_x: 900,  off_y: 580, nx: 752, ny: 427 },
        Region { name: "southeast",             off_x: 900,  off_y:  80, nx: 708, ny: 539 },
        Region { name: "northeast",             off_x: 1200, off_y: 550, nx: 575, ny: 464 },
    ]
}
...
fn build_conus_volume(hour: usize) -> ConusVolume {
    ...
    ConusVolume { nx: CONUS_NX, ny: CONUS_NY, nz, pressure, t }
}
...
fn build_conus_slab(hour: usize) -> ConusSlab {
    ...
    let mk = |seed: f64, amp: f64, base: f64| -> Vec<f64> {
        (0..n).map(|i| base + amp * (((i as f64) * seed + phase).sin())).collect()
    };
    ConusSlab {
        nx: CONUS_NX, ny: CONUS_NY,
        ...
        dx: vec![HRRR_DX_M; n],
        dy: vec![HRRR_DX_M; n],
        lat: (0..n).map(|i| 25.0 + ((i / CONUS_NX) as f64) * 0.025).collect(),
    }
}
...
#[derive(Clone, Copy)]
enum Cat { VInterp, Severe, Stencil, Thermo, Wind, Transport }
impl Cat {
