//! Moist-air gas constant `R = Rd * (1 + w/eps) / (1 + w)` (J/(kg*K)) —
//! port of met-cu's `moist_air_gas_constant_kernel`. Matches
...
/// Moist-air gas constant (J/(kg*K)) from mixing ratio (kg/kg).
pub fn host(ctx: &ContextHandle, mixing_ratio: &[f64]) -> Result<Vec<f64>> {
    ...
    let mut r_d: DeviceVec<f64> = DeviceVec::zeros(ctx, n)?;
