//! Relative vorticity `dv/dx - du/dy` — port of met-cu's `vorticity_kernel`.
//! Matches `wx_math::dynamics::vorticity` (when `dx`/`dy` are constant fields).
...
const KERNEL_SRC: &str = include_str!("../../../kernels/grid/vorticity.cu");
const MODULE_KEY: &str = "grid_vorticity";
const FUNCTION: &str = "vorticity_kernel";
...
/// Relative vorticity on a row-major `(ny, nx)` field.
/// `dx`/`dy` are per-point spacings (length `ny*nx`).
pub fn host(
    ctx: &ContextHandle,
    u: &[f64],
    v: &[f64],
