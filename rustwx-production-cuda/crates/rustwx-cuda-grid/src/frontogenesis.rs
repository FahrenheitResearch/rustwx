//! Petterssen 2D frontogenesis — port of met-cu's `frontogenesis_kernel`.
//! Matches `wx_math::dynamics::frontogenesis_2d`.
...
const KERNEL_SRC: &str = include_str!("../../../kernels/grid/frontogenesis.cu");
const MODULE_KEY: &str = "grid_frontogenesis";
const FUNCTION: &str = "frontogenesis_kernel";
...
pub fn host(
    ...
    let n = nx * ny;
    if theta.len() != n {
        return Err(Error::LengthMismatch { what: "theta vs nx*ny", expected: n, got: theta.len() });
    }
    if u.len() != n {
    ...
    let theta_d = DeviceVec::from_host(ctx, theta)?;
