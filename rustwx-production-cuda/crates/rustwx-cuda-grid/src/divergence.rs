//! Horizontal divergence `du/dx + dv/dy` — port of met-cu's `divergence_kernel`.
//! Matches `wx_math::dynamics::divergence` (when `dx`/`dy` are constant fields).
...
const KERNEL_SRC: &str = include_str!("../../../kernels/grid/divergence.cu");
const MODULE_KEY: &str = "grid_divergence";
const FUNCTION: &str = "divergence_kernel";
