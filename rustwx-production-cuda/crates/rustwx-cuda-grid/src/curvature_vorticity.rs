//! Curvature vorticity — the component of vorticity arising from streamline
//! curvature. Port of met-cu's `curvature_vorticity_kernel`. Matches
//! `wx_math::dynamics::curvature_vorticity` (when `dx`/`dy` are constant fields).
...
const KERNEL_SRC: &str = include_str!("../../../kernels/grid/curvature_vorticity.cu");
const MODULE_KEY: &str = "grid_curvature_vorticity";
const FUNCTION: &str = "curvature_vorticity_kernel";
...
/// Curvature vorticity on a row-major `(ny, nx)` field.
pub fn host(
    ctx: &ContextHandle,
    u: &[f64],
    v: &[f64],
