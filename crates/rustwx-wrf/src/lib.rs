...
impl WrfFile {
    ...
    pub fn td2(&self) -> Result<SharedField, WrfError> {
        self.cached_or_compute("td2", || {
            if self.has_var("TD2") {
                return self.read_var("TD2");
            }
            Ok(self
                .q2()?
                .iter()
    ...
    pub fn reflectivity_1km_agl(&self) -> Result<SharedField, WrfError> {
        self.cached_or_compute("reflectivity_1km_agl", || {
            if self.has_var("REFL_1KM_AGL") {
                return self.read_var("REFL_1KM_AGL");
            }
            self.require_3d()?;
            Ok(interp_to_height_level(
                &self.dbz_3d()?,
                &self.height_agl()?,
                self.nx,
                self.ny,
