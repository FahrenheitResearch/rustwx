...
mod tests {
    ...
    fn intersect_bounds_rejects_disjoint_boxes() {
        assert!(
            intersect_bounds([-100.0, 30.0, -90.0, 40.0], [-80.0, 30.0, -70.0, 40.0]).is_none()
        );
        assert_eq!(
    ...
    fn product_qc_reports_generic_moment_value_range() {
        ...
        let summary =
            radar_product_qc_summary(&sweep, RadarProduct::CorrelationCoefficient).unwrap();
    ...
    fn reflectivity_despeckle_removes_isolated_gate_only() {
        ...
        let (filtered, summary) =
            despeckle_reflectivity_sweep(&sweep, RadarProduct::Reflectivity, 1);
        let row0 =
            radial_moment_for_product(&filtered.radials[0], RadarProduct::Reflectivity).unwrap();
        let row1 =
            radial_moment_for_product(&filtered.radials[1], RadarProduct::Reflectivity).unwrap();
