...
fn calibration_report_prefers_repeated_holdout_benchmark() {
    ...
    let deduped_history =
        merge_surface_mesoanalysis_innovation_history(Some(history.clone()), history.clone(), None);
...
fn calibration_gate_can_require_case_diversity() {
    let duplicate_signature_report = build_surface_mesoanalysis_calibration_report_from_values([
        (
