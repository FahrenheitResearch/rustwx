...
fn compute_ecape_triplet_fields(
    ...
    for field in fields {
        match field.product {
            Solar07Product::Sbecape => sbecape_jkg = Some(field.values),
            Solar07Product::Mlecape => mlecape_jkg = Some(field.values),
            Solar07Product::Muecape => muecape_jkg = Some(field.values),
            _ => {}
        }
    }
