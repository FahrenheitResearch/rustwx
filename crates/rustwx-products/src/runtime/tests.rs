...
fn rrfs_pair_fetch_requests_use_subset_patterns() {
    ...
    let nat_request = build_fetch_request(&plan, &nat_key, None).expect("nat-na request builds");
    let prs_request = build_fetch_request(&plan, &prs_key, None).expect("prs-na request builds");
    assert!(
        nat_request
            .variable_patterns
