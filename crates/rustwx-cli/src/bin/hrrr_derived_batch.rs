...
#[derive(Debug, Parser)]
#[command(
    name = "hrrr-derived-batch",
    about = "Generate multiple derived RustWX HRRR plots from one shared timestep decode"
)]
struct Args {
...
fn run(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = HrrrDerivedBatchRequest {
    ...
    let stem = format!(
        "rustwx_hrrr_{}_{}z_f{:03}_{}_derived",
    ...
    let mut run_manifest =
        RunPublicationManifest::new("hrrr_derived_batch", stem.clone(), args.out_dir.clone())
            .with_run_metadata(
                "hrrr",
