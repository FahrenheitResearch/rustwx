...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fields = compute_panel_fields(&surface, &pressure)?;
...
) -> Result<Vec<PanelField>, Box<dyn std::error::Error>> {
...
fn panel_fields_from_supported(fields: SupportedSevereFields) -> Vec<PanelField> {
    vec![
        PanelField {
            product: Solar07Product::Sbcape,
            title_override: None,
            units: "J/kg",
            values: fields.sbcape_jkg,
        },
        PanelField {
            product: Solar07Product::Mlcin,
            title_override: None,
            units: "J/kg",
            values: fields.mlcin_jkg,
        },
        PanelField {
            product: Solar07Product::Mucape,
            title_override: None,
            units: "J/kg",
            values: fields.mucape_jkg,
        },
        PanelField {
            product: Solar07Product::Srh01km,
            title_override: None,
            units: "m^2/s^2",
            values: fields.srh_01km_m2s2,
        },
        PanelField {
            product: Solar07Product::Srh03km,
            title_override: None,
            units: "m^2/s^2",
            values: fields.srh_03km_m2s2,
        },
        PanelField {
            product: Solar07Product::StpFixed,
            title_override: None,
            units: "dimensionless",
            values: fields.stp_fixed,
        },
        PanelField {
            product: Solar07Product::Scp,
            title_override: Some("SCP (MU / 0-3 KM / 0-6 KM PROXY)"),
            units: "dimensionless",
            values: fields.scp_mu_03km_06km_proxy,
        },
        PanelField {
            product: Solar07Product::Ehi,
            title_override: Some("EHI (SB / 0-1 KM PROXY)"),
            units: "dimensionless",
            values: fields.ehi_sb_01km_proxy,
        },
    ]
}
...
fn render_panel(
    ...
    let run_title = format!(
        "HRRR Severe Proof Panel  Run: {date} {cycle:02}:00 UTC  Forecast Hour: F{forecast_hour:02}"
    );
    let subtitle_1 = "STP is fixed-layer only: sbCAPE + sbLCL + 0-1 km SRH + 0-6 km bulk shear.";
    let subtitle_2 = "SCP and EHI are fixed-depth proxies here: SCP uses muCAPE + 0-3 km SRH + 0-6 km shear. EHI uses sbCAPE + 0-1 km SRH. Effective-layer derivation is not wired yet.";
    ...
    text::draw_text_centered(&mut canvas, subtitle_1, 34, wrf_render::Rgba::BLACK, 1);
    text::draw_text_centered(&mut canvas, subtitle_2, 54, wrf_render::Rgba::BLACK, 1);
...
mod tests {
    ...
    fn severe_panel_products_keep_fixed_and_proxy_labels_explicit() {
        ...
        assert_eq!(
            fields[6].title_override,
            Some("SCP (MU / 0-3 KM / 0-6 KM PROXY)")
        );
        assert_eq!(fields[7].title_override, Some("EHI (SB / 0-1 KM PROXY)"));
    }
}
