use rustwx_render_verify::{
    Color, PanelGridLayout, RgbaImage, Solar07Product, count_non_background_pixels,
    ...
    sample_solar07_request, save_png,
};
use std::error::Error;
...
fn verify_harness_can_write_png_artifacts() -> Result<(), Box<dyn Error>> {
    let request = sample_solar07_request(Solar07Product::Scp)?;
