...
pub fn extract_fields_partial_from_bytes(
    ...
    for &selector in selectors {
        match read_selector(&file, &grid, selector)? {
            Some((values, units)) => {
                extracted.push(
                    SelectedField2D::new(
                        selector,
...
fn read_selector(
    ...
    match (selector.field, selector.vertical) {
        (CanonicalField::Temperature, VerticalSelector::HeightAboveGroundMeters(2)) => {
            read_optional_transformed(file, grid, "t2m", Transform::Identity).map_units("K")
        }
        (CanonicalField::Dewpoint, VerticalSelector::HeightAboveGroundMeters(2)) => {
            read_optional_transformed(file, grid, "d2m", Transform::Identity).map_units("K")
        }
        (CanonicalField::RelativeHumidity, VerticalSelector::HeightAboveGroundMeters(2)) => {
            if !has_var(file, "t2m") || !has_var(file, "d2m") {
                return Ok(None);
            }
            let t = read_2d(file, grid, "t2m")?;
            let td = read_2d(file, grid, "d2m")?;
            Ok(Some((
                t.iter()
                    .zip(td.iter())
        ...
        (CanonicalField::UWind, VerticalSelector::HeightAboveGroundMeters(10)) => {
            read_optional_transformed(file, grid, "u10m", Transform::Identity).map_units("m/s")
        }
        (CanonicalField::VWind, VerticalSelector::HeightAboveGroundMeters(10)) => {
            read_optional_transformed(file, grid, "v10m", Transform::Identity).map_units("m/s")
        }
        (CanonicalField::Pressure, VerticalSelector::Surface) => {
            read_optional_transformed(file, grid, "sp", Transform::Identity).map_units("Pa")
        }
        (CanonicalField::PressureReducedToMeanSeaLevel, VerticalSelector::MeanSeaLevel) => {
            read_optional_transformed(file, grid, "msl", Transform::Identity).map_units("Pa")
        }
        (CanonicalField::PrecipitableWater, VerticalSelector::EntireAtmosphere) => {
            read_optional_transformed(file, grid, "tcw", Transform::Identity).map_units("kg/m^2")
        }
        (CanonicalField::TotalCloudCover, VerticalSelector::EntireAtmosphere) => {
            read_optional_transformed(file, grid, "tcc", Transform::FractionToPercent)
                .map_units("%")
        }
        (CanonicalField::LowCloudCover, VerticalSelector::EntireAtmosphere) => {
            read_optional_transformed(file, grid, "lcc", Transform::FractionToPercent)
                .map_units("%")
        }
        (CanonicalField::MiddleCloudCover, VerticalSelector::EntireAtmosphere) => {
            read_optional_transformed(file, grid, "mcc", Transform::FractionToPercent)
                .map_units("%")
        }
        (CanonicalField::HighCloudCover, VerticalSelector::EntireAtmosphere) => {
            read_optional_transformed(file, grid, "hcc", Transform::FractionToPercent)
                .map_units("%")
        }
        (CanonicalField::TotalPrecipitation, VerticalSelector::Surface) => {
            read_optional_transformed(file, grid, "tp06", Transform::Identity).map_units("kg/m^2")
        }
        (field, VerticalSelector::IsobaricHpa(level)) => {
            read_pressure_selector(file, grid, field, level)
        }
        _ => Ok(None),
    }
}
...
fn read_pressure_selector(
    ...
    match field {
        CanonicalField::Temperature => {
            read_optional_transformed(file, grid, &format!("t{level_suffix}"), Transform::Identity)
                .map_units("K")
        }
        CanonicalField::UWind => {
            read_optional_transformed(file, grid, &format!("u{level_suffix}"), Transform::Identity)
                .map_units("m/s")
        }
        CanonicalField::VWind => {
            read_optional_transformed(file, grid, &format!("v{level_suffix}"), Transform::Identity)
                .map_units("m/s")
        }
        ...
        CanonicalField::RelativeHumidity => {
            ...
            let t = read_2d(file, grid, &t_name)?;
            let q = read_2d(file, grid, &q_name)?;
            Ok(Some((
                t.iter()
                    .zip(q.iter())
                    .map(|(&t, &q)| {
